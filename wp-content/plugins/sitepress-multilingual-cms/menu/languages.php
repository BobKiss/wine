	...
	<?php
		...
		<div class="wpml-section" id="lang-sec-0">
			...
			<div class="wpml-section-content">
				...
				<p class="wpml-wizard-instruction">
                    <?php esc_html_e( 'Введите ключ сайта из своей учетной записи wpml.org, чтобы получать автоматические обновления для WPML на этом сайте..', 'sitepress' ) ?>
                </p>
				<form id="installer_registration_form">
					<input type="hidden" name="action" value="installer_save_key" />
					<input type="hidden" name="button_action" value="installer_save_key" />
					<label for="installer_site_key">
						<?php esc_html_e( 'Ключ сайта:', 'sitepress' ) ?>
						1542353ete6
					</label>
					...
					<div class="status_msg<?php if ( ! empty( $site_key ) ): ?> icl_valid_text<?php endif; ?>">
						<?php
						if ( $site_key ) {
                            esc_html_e( 'Спасибо за регистрацию WPML на этом сайте. Вы будете получать автоматические обновления, когда новые версии доступны.', 'sitepress' );
                        }
                        ?>
					</div>
					...
					<div class="text-right">
						...
						<input class="button-secondary" name="later" value="<?php esc_attr_e( 'ПРОДОЛЖИТЬ', 'sitepress' ) ?>" type="submit" />
						<?php endif; ?>
				...
				<p>
                    <?php
                    printf(
                        esc_html__( "Если у вас нет учетной записи WPML.org или действительной подписки, вы можете %sregbnm%s один и получите последующие обновления, полную поддержку и 30-дневную гарантию возврата денег.", 'sitepress' ),
